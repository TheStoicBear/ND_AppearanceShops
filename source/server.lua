lib.callback.register(
    "ND_AppearanceShops:clothingPurchase",
    function(src, store, clothing)
...
RegisterNetEvent(
    "ND_AppearanceShops:updateAppearance",
    function(clothing)
        local src = source
        local player = NDCore.getPlayer(src)
        if not player then
...
AddEventHandler(
    "ND_AppearanceShops:saveCharacterOutfit",
    function(src, clothingData)
        local src = source
        ...
        MySQL.Async.execute(
            "UPDATE nd_characters SET clothing = @clothing WHERE charid = @charid",
            {
...
AddEventHandler(
    "ND_AppearanceShops:getCharacterOutfit",
    function()
        local src = source
        local player = NDCore.getPlayer(src) -- Retrieve player object
        ...
        print("Server received outfit request from player: " .. player.firstname .. " " .. player.lastname)

        -- Retrieve clothing data from the database
        MySQL.Async.fetchScalar(
            "SELECT clothing FROM nd_characters WHERE charid = @charid",
            {
                ["@charid"] = player.id
            },
            function(clothingDataJson)
                if clothingDataJson then
                    print("Clothing data found, sending to client...")
